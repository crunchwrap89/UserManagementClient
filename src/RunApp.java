	...
	private static void idSearch() {
		while (true) {
			...
			if (sc2.hasNextInt() == false) {
				...
				System.out.println("Metod för idsökning inte inlagd än");
				break;
			}
		}
	}
...
//		System.out.println(s);
	...
	public static void addUser(String fornamn, String efternamn) {
		...
		Response response = client.target("http://localhost:8080/UserManagement/webservice/users").request().buildPost(alfredity)
				.invoke();
//		System.out.println(response.readEntity(User1.class).getId());
	...
	public static void listAll() {
		Client client = ClientBuilder.newClient();
		Response response = client.target("http://localhost:8080/UserManagement/webservice/users").request().buildGet().invoke();
